...
const fs = require("fs")
const animation = JSON.parse(fs.readFileSync("./public/data/animation.json"))
const entity = JSON.parse(fs.readFileSync("./public/data/entity.json"))
...
exports.Entity = class {
    ...
    rotate(r) {
       this.animation.rotation += r;
        ...
        let centerX = this.hitbox.x + this.hitbox.width/2;
        let centerY = this.hitbox.y + this.hitbox.height/2;

        let dx = centerX - rotX;
        let dy = centerY - rotY;
        let rads = (r / 180) * Math.PI;
        let x = (dx * Math.cos(rads)) - (dy * Math.sin(rads));
        let y = (dx * Math.sin(rads)) + (dy * Math.cos(rads));
        this.hitbox.x = x + rotX - this.hitbox.width/2;
        this.hitbox.y = y + rotY - this.hitbox.height/2;
    }
    setRotation(r) {
        let rotDiff = r - this.animation.rotation;
        this.animation.rotation = r;
         // get center coordinates
         let rotX = this.x;
         let rotY = this.y;
